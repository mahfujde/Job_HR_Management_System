...
<body style= "font-family Times New Roman,serif;background-color:#E5E4E2;">
      ...
      @if(empty($coverpictures))
        ...
        <form action="{{url('/save/coverPicture')}}" method="post" enctype="multipart/form-data">
        	 {{csrf_field()}}
      ...
      @else
       ...
       <div class="card card-picture">
      <div class="card-body m-3">
        <form action="{{url('/update/coverPicture')}}" method="post" enctype="multipart/form-data">
           {{csrf_field()}}
        <label class="col-md-12 control-label" for="about_me">
      <h5>Update Cover Picture:</h5></label>
      <hr/>
        <label class="custom-file m-3">
           <input type="file" name="cover_picture" class="choose-pictire" placeholder="{{$coverpicture->cover_picture}}">
          </label>
          <hr/>
