...
<body style= "font-family Times New Roman,serif;background-color:#E5E4E2;">
      ...
      @if(empty($company_coverpictures))
        <div class="card card-picture">
      <div class="card-body m-3">
        <form action="{{url('/save/companyCoverPicture')}}" method="post" enctype="multipart/form-data">
           {{csrf_field()}}
        <label class="col-md-12 control-label" for="about_me">
      <h5>Add Cover Picture:</h5></label>
      <hr/>
        <label class="custom-file m-3">
           <input type="file" name="cover_picture" class="choose-pictire">
          </label>
          <hr/>
    ...
    <a href="{{url('/companyProfile')}}" class="btn btn-danger col-md-3">Cancle</a>
      ...
      @foreach($company_coverpictures as $company_coverpicture)
       <div class="card card-picture">
      <div class="card-body m-3">
        <form action="{{url('/update/companyCoverPicture')}}" method="post" enctype="multipart/form-data">
           {{csrf_field()}}
        <label class="col-md-12 control-label" for="cover_picture">
      <h5>Update Cover Picture:</h5></label>
      <hr/>
        <label class="custom-file m-3">
           <input type="file" name="cover_picture" class="choose-pictire">
          </label>
